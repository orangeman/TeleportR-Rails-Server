...
set :application, "teleportr"

# this is used for rvm 
set :rvm_ruby_string, 'ree@teleportr'
set :rvm_type, :user

set :scm, :git
set :repository,  "git://dev.c-base.org/teleportr/server.git"

# on the remote host
set :applicationdir, "/var/www/teleportr"
set :config_dir, "/etc/rails/teleportr"
set :user , "teleportr"
set :group , "teleportr"
set :server, :unicorn
...
namespace :deploy do
  task :start, :roles => :app, :except => { :no_release => true } do 
    run "cd #{current_path} && RAILS_ENV=#{rails_env} #{try_sudo} #{unicorn_binary} -c #{unicorn_config} -E #{rails_env} -D"
  end
