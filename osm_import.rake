...
#			%x[#{psql} -f /usr/share/postgresql/8.4/contrib/hstore.sql]
			...
			#country = "hamburg" # for test reason
			#`wget http://download.geofabrik.de/osm/europe/#{country}.osm.bz2 \
			#			-O #{download_path}/#{country}.osm.bz2`
			puts "osmosis running..."
  		%x[#{osmosis_binary} \
				    --read-xml #{download_path}/#{country}.osm.bz2 \
 ...
 # 	end
  ...
  conn = get_osm_db("tele_test","germany")
	puts
...
SQL
		...
		Place.create :name => s["v"], :latlon => node.first["geom"], :modes => STREET
		#places << p
	end
